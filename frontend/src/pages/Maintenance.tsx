...
const Maintenance: React.FC = () => {
  ...
  const fetchMaintenanceData = async () => {
    try {
      const [ordersRes, dvirsRes, incidentsRes] = await Promise.all([
        api.get("/maintenance"),
        api.get("/maintenance/dvir"),
        api.get("/maintenance/incidents"),
      ]);
      setWorkOrders(ordersRes.data);
      setDvirLogs(dvirsRes.data);
      setIncidentLogs(incidentsRes.data);
    } catch (e) {
      console.error("fetchMaintenanceData error:", e);
    }
  };
  ...
  const onOrderSubmit = async (data: WorkOrderFormValues) => {
    try {
      ...
      toast.error(e.message || "Failed to submit work order.");
    }
  };

  const onDvirSubmit = async (data: DvirFormValues) => {
    const pass = data.brakesPass && data.lightsPass && data.steeringPass && data.tiresPass;
    try {
      await api.post("/maintenance/dvir", {
      ...
      toast.error(e.message || "Failed to submit DVIR inspection.");
    }
  };
  ...
  const onIncidentSubmit = async (data: IncidentFormValues) => {
    try {
      ...
      toast.error(e.message || "Failed to submit incident log.");
    }
  };

  const handleToggleOrderStatus = async (id: number, status: "IN_PROGRESS" | "COMPLETED") => {
    try {
      if (status === "COMPLETED") {
      ...
      toast.error(e.message || "Failed to update work order status.");
    }
  };
